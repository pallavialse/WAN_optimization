...
void hash_addPhraseAt(long hashId,const char *phrase, uchar phraseLen)
{
...
void hash_retrieveValue(long hashId,char *phrase,uchar *phraseLen)
{
	int bucketId=HASHID_TO_BUCKET(hashId);
	int subIndex=HASHID_TO_SUBINDEX(hashId);
	nodes *node,*prev,*next;
	
	if(hasht->table[bucketId].list==NULL) {
		printf("error condition");
	}
	
	if(hasht->table[bucketId].list!=NULL) {
		if(subIndex==0) {
			memcpy(phrase,hasht->table[bucketId].list->value,hasht->table[bucketId].list->length);
		} else if(subIndex > 0) {
			node=hasht->table[bucketId].list;
			while(subIndex > 0) {
					prev=node;
					node=node->next;
					subIndex--;
				}
			memcpy(phrase,node->value,node->length);
			*phraseLen=node->length;
		} else {
			printf("error condition");
		}	
	}	
